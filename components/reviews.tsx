    ...
    <section id="reviews" className="bg-background py-24 md:py-32">
      <div className="mx-auto max-w-6xl px-6">
        <div className="grid gap-12 md:grid-cols-[0.85fr_1.15fr] md:gap-16">
          <div>
            <span className="text-xs font-medium uppercase tracking-[0.4em] text-accent">Reviews</span>
            <h2 className="mt-5 text-balance font-serif text-3xl font-semibold leading-snug text-foreground sm:text-4xl">
              손님들이 남겨주신 이야기
            </h2>
            <p className="mt-6 text-pretty leading-relaxed text-muted-foreground">
              삼성중앙역 근처, 모던한 외관이 매력적인 오더에스프레소. 산미 없이 묵직한 아메리카노와
              따뜻하게 내어드리는 소금빵·에그타르트가 특히 사랑받고 있습니다.
            </p>
            <blockquote className="mt-8 border-l-2 border-accent pl-5 text-pretty font-serif text-lg italic leading-relaxed text-foreground">
              &ldquo;커피가 맛있어요&rdquo; 그리고 &ldquo;디저트가 맛있어요&rdquo; &mdash; 방문객들이
              가장 많이 남긴 한마디.
            </blockquote>
          </div>
          ...
          <ul className="flex flex-col justify-center gap-4">
            {keywords.map((k) => (
              <li key={k.label} className="flex items-center gap-4">
                <span className="w-40 shrink-0 text-sm text-foreground">{k.label}</span>
                ...
                <span className="w-10 shrink-0 text-right text-sm tabular-nums text-muted-foreground">{k.count}</span>
              </li>
            ))}
          </ul>
        </div>
